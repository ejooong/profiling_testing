...
@section('title', 'Edit Berita: ' . $berita->title)
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Edit berita: {{ $berita->title }}</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="dpc_id" 
                                ...
                                @foreach($dpcs as $dpc)
                                <option value="{{ $dpc->id }}" {{ old('dpc_id', $berita->dpc_id) == $dpc->id ? 'selected' : '' }}>
                                    {{ $dpc->kecamatan_name }}
                                </option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="w-64 h-48 rounded-lg overflow-hidden border">
                ...
                <div>
                    ...
                    <textarea name="excerpt" 
                              ...
                              rows="3" 
                              required
                              class="w-full border-gray-300 rounded-md shadow-sm focus:border-nasdem-red focus:ring focus:ring-red-200 focus:ring-opacity-50">{{ old('excerpt', $berita->excerpt) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="content" 
                              ...
                              rows="15" 
                              required
                              class="w-full border-gray-300 rounded-md shadow-sm focus:border-nasdem-red focus:ring focus:ring-red-200 focus:ring-opacity-50">{{ old('content', $berita->content) }}</textarea>
                    @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   name="meta_keywords" 
                                   id="meta_keywords" 
                                   value="{{ old('meta_keywords', is_array($berita->meta_keywords) ? implode(', ', $berita->meta_keywords) : $berita->meta_keywords) }}"
                                   class="w-full border-gray-300 rounded-md shadow-sm focus:border-nasdem-red focus:ring focus:ring-red-200 focus:ring-opacity-50">
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" 
                                      id="meta_description" 
                                      rows="3"
                                      class="w-full border-gray-300 rounded-md shadow-sm focus:border-nasdem-red focus:ring focus:ring-red-200 focus:ring-opacity-50">{{ old('meta_description', $berita->meta_description) }}</textarea>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t flex justify-between">
                <div>
                    <button type="button" 
                            onclick="if(confirm('Apakah Anda yakin ingin menghapus berita ini?')) document.getElementById('delete-form').submit();"
        ...
        <!-- Delete Form -->
        <form id="delete-form" action="{{ route('admin.berita.destroy', $berita) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
charCount.innerHTML = `${metaDescription.value.length}/160 karakter`;
